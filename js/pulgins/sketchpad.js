class Sketchpad {
  constructor(options) {
    ...
    this.internalEvents = ['MouseDown', 'MouseUp', 'MouseOut','TouchStart','TouchMove','TouchEnd'];
  ...
  _position(event) {
  	...
  	var pageY = event.touches ? event.touches[0].pageX : event.pageY ;
    return {
  ...
  onMouseDown(event) {
    ...
    this._currentStroke = {
      ...
      lines: [],
		...
		var type = event.touches ? 'touchmove' : 'mousemove' ;
    this.canvas.addEventListener(type, this.onMouseMove);
  }
  ...
  onMouseUp(event) {
    if (this._sketching) {
      ...
      this._sketching = false;
		...
		var type = event.touches ? 'touchmove' : 'mousemove' ;
    this.canvas.removeEventListener(type, this.onMouseMove);
  }
